...
import { FaBookmark, FaRegFolderOpen } from "react-icons/fa";
  ...
  useEffect(() => {
    const fetchSavedVideos = async () => {
      ...
      try {
        ...
        showCustomAlert("Failed to load saved videos.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="w-full min-h-screen bg-[#0f0f0f] text-white pt-16 pb-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="border-b border-[#2d2d2d] pb-4 flex items-center gap-3">
          <FaBookmark className="text-purple-400 text-lg" />
          <h1 className="text-2xl font-bold tracking-tight text-white">Saved Videos</h1>
        </div>
        ...
        {videos.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-20 space-y-5">
            ...
            <div className="space-y-1.5">
              <h2 className="text-xl font-bold text-gray-300">No saved videos yet</h2>
              <p className="text-sm text-gray-500 max-w-sm">
                Videos you bookmark will be added here for quick access. Go explore some content!
              </p>
            </div>
            <Link
              to="/"
