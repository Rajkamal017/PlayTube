...
import { FaThumbsUp, FaRegFolderOpen } from "react-icons/fa";
...
const LikedVideos = () => {
  ...
  useEffect(() => {
    const fetchLikedVideos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${serverUrl}/api/content/liked-videos`, {
          withCredentials: true,
        });
        setVideos(response.data.videos || []);
      } catch (error) {
        console.error("Error fetching liked videos:", error);
        showCustomAlert("Failed to load liked videos.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="w-full min-h-screen bg-[#0f0f0f] text-white pt-16 pb-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="border-b border-[#2d2d2d] pb-4 flex items-center gap-3">
          <FaThumbsUp className="text-purple-400 text-lg" />
          <h1 className="text-2xl font-bold tracking-tight text-white">Liked Videos</h1>
        </div>
        ...
        {videos.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-20 space-y-5">
            ...
            <div className="space-y-1.5">
              <h2 className="text-xl font-bold text-gray-300">No liked videos yet</h2>
              <p className="text-sm text-gray-500 max-w-sm">
                Videos you like will be added here for quick access. Go explore some content!
              </p>
            </div>
            <Link
              to="/"
