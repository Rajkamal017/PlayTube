...
import create from "../assets/createV2.svg";
...
const CreatePage = () => {
  ...
  // handle route by selected state
  const handleRoute = () => {
    const routes = {
      video: "/createvideo",
      short: "/createshort",
  ...
  return (
    <div className="bg-[#0f0f0f] min-h-screen text-white px-6 py-8 mt-10 flex flex-col">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 flex-1">
        {options.map((opt) => (
          <div
            className={`bg-[#1f1f1f] border border-[#3f3f3f] rounded-lg p-6 flex flex-col items-center text-center justify-center cursor-pointer transition ${selected === opt.id ? "ring-2 ring-purple-500" : "hover:bg-[#272727]"}`}
      ...
      <div className="flex flex-col items-center mt-16">
        <img src={create} alt="" className="w-20 rounded-2xl" />
        {!selected ? (
          <div>
          ...
          <div className="flex flex-col items-center justify-center gap-1">
            ...
            <button className="bg-white text-black mt-4 px-5 py-1 rounded-full font-medium cursor-pointer" onClick={handleRoute}>
              + Create
            </button>
          </div>
        )}
